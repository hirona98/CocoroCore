...
class RequestHookProcessor:
    ...
    async def _process_message(self, request: Any) -> None:
        ...
        if is_text_chat:
            ...
            if self.cocoro_dock_client:
                asyncio.create_task(
                    self.cocoro_dock_client.send_chat_message(role="user", content=request.text)
                )
        ...
        if self.cocoro_shell_client:
            asyncio.create_task(
                self.cocoro_shell_client.send_control_command(command="lookForward")
            )
        ...
        if self.wakewords:
            for wakeword in self.wakewords:
                if wakeword.lower() in request.text.lower():
                    ...
                    if self.cocoro_dock_client:
                        asyncio.create_task(
                            self.cocoro_dock_client.send_status_update(
    ...
    async def _start_llm_status_notifications(self, request: Any) -> None:
        ...
        # 初回のステータス通知
        asyncio.create_task(
            self.cocoro_dock_client.send_status_update("LLM API呼び出し", status_type="llm_sending")
        )
